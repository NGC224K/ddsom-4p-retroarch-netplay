...
cls
echo ================================================================
echo   DND Shadow over Mystara - 4인 온라인 도우미
echo ================================================================
echo   [1] 방 참가 - 고화질
echo   [2] 방 만들기 - 고화질
echo   [3] 처음사용설명서 열기
...
echo.
set "HOST="
set /p "HOST=방장에게 받은 주소(IP 또는 도메인): "
if not defined HOST goto menu
set "NICK="
set /p "NICK=게임에서 사용할 이름 [Friend]: "
if not defined NICK set "NICK=Friend"
if not exist "%~dp0logs" mkdir "%~dp0logs"
echo 접속 중입니다. Windows 방화벽 질문이 나오면 액세스를 허용하세요.
echo 접속 순서대로 첫 친구=P2, 둘째=P3, 셋째=P4입니다.
...
:join_done
echo RetroArch가 닫혔습니다. 오류가 있으면 logs\last-join.log를 방장에게 보내세요.
pause
goto menu
...
echo.
if defined PUBLIC_IP echo 공인 IPv4: !PUBLIC_IP!
if not defined PUBLIC_IP echo 공인 IPv4 자동 확인에 실패했습니다.
echo 접속 포트: TCP 55435
set "NICK="
set /p "NICK=방에 표시할 이름 [Host]: "
if not defined NICK set "NICK=Host"
if not exist "%~dp0logs" mkdir "%~dp0logs"
echo Windows 방화벽 질문이 나오면 개인 네트워크 액세스를 허용하세요.
...
:rom_missing
echo roms\ddsom.zip이 없습니다. 합법적으로 보유한 ROM ZIP을 넣으세요.
pause
goto menu

:rom_failed
echo ROM 검사가 실패했습니다. SHA-256 값을 방장과 비교하세요.
pause
goto menu
